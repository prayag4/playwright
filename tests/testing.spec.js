...
test.describe("Verify CRUD screen", () => {
    test.beforeAll("generate utility functions", () => {
        objRandomUtility = new RandomUtility()
    })
    test("verify add case", async ({ page }) => {
        objBasePage = new BasePage(page)
        ...
        //editor
        let iframe = page.frameLocator(editorIframeSelector)
        ...
        //single selection
        await page.locator(singleSelectionSelector).selectOption({ value: formData.singleSelection })
        await page.locator(multiSelectionSelector).selectOption(formData.multiSelection);
        await page.locator(checkboxSelector).check(formData.checkbox)
